...
class Pymongo_:
    ...
    def execution(self,Choice):        
        if Choice==1:
            ...
            i=1
            while i<=n:
                key=input("Enter column %s's name:" %i)
                value=str(input("Enter its value:"))
                a[key]=value
                i+=1
            ...
            Obj.current_docs()
            updtkey=input("Please enter the unique key column on which you you want to update:")
            keyval=input("Please enter the value of unique key column:")
            updcol=input("Please enter the update column:")
            newval=input("Please enter new value:")
            result = self.collection.update_many({updtkey:keyval},{"$set":{updcol:newval}})
            Obj.current_docs()   
